...
Reading `SKILL.md` gives the skill body without its frontmatter, wrapped in `<skill_content name="...">`, followed by the skill directory and a `<skill_resources>` listing of the other files in the skill. The listing skips hidden files, `__pycache__`, and `node_modules`, and shows up to 50 files. Other files are returned in a `<skill_file name="..." path="...">` wrapper and truncated after 2^16 characters. Only UTF-8 text files up to 4 MB can be read. Paths that leave the skill directory or refer to hidden files, `__pycache__`, or `node_modules` are rejected. Errors are returned to the LLM as `"Error: ..."` strings, while the tool call box shows the corresponding `Failure`.

Symbolic links are resolved with `AbsoluteFileName` before reading, since a skill downloaded from elsewhere (e.g. a cloned repository) can contain links to any file on the machine. A file can only be read if its resolved location is inside the resolved skill directory and is not a hidden or ignored file, and the same applies to `SKILL.md` when a skill is loaded. The listing skips links that don't meet this condition and doesn't follow linked directories. The skill directory itself can be a link.
