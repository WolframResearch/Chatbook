...
notebookEdit[ as_Association ] := (Global`as = as; notebookEdit[ as[ "action" ], as[ "target" ], as[ "content" ] ]);
...
notebookEditTarget[ "selected" ] /; $WorkspaceChat := Global`selected = getUserNotebook[ ];
notebookEditTarget[ "selected" ] /; $InlineChat := Global`selected = ensureValidWriteTarget @ $inlineChatState[ "ParentCell" ];
notebookEditTarget[ "selected" ] /; True := Global`selected = $evaluationNotebook;
